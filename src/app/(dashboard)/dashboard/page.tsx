...
export default function DashboardPage() {
  ...
  const handleGenerateBusiness = useCallback(async (p: string) => {
    ...
    // Status sequence for UX
    const statuses = [
      "Brainstorming Brand Name...",
      "Designing Logo & Website...",
      "Drafting Outreach Scripts...",
      "Configuring AI Sales Manager...",
      "Setting up CRM Dashboard...",
      "Finding First 100 Leads..."
    ];
    
    let statusIndex = 0;
    const statusInterval = setInterval(() => {
      if (statusIndex < statuses.length) {
        setGenStatus(statuses[statusIndex]);
        statusIndex++;
      }
    }, 3000);
  ...
  useEffect(() => {
    if (promptParam && !isGenerating && !loading) {
      handleGenerateBusiness(promptParam);
    }
    ...
    <div className="p-8 max-w-7xl mx-auto w-full relative">
      ...
      <AnimatePresence>
        {isGenerating && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/90 backdrop-blur-xl transition-all p-6 text-center"
          >
            <div className="relative mb-8">
              <div className="absolute inset-0 bg-white/10 blur-3xl rounded-full"></div>
              <Bot size={80} className="text-white relative animate-pulse" />
            </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
        ...
        <button 
          ...
          className="px-4 py-2 bg-white text-black rounded-full text-sm font-bold flex items-center gap-2 hover:bg-gray-200 transition-colors w-fit"
        >
        ...
        {showNewModal && (
          ...
          >
            <motion.div 
              ...
              className="w-full max-w-lg bg-[#0a0a0a] border border-white/10 p-8 rounded-3xl shadow-2xl"
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase mb-2 block">Business Idea</label>
                  <textarea 
                    autoFocus
                    placeholder="e.g. A content marketing agency for sustainable fashion brands..."
                    className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-sm outline-none focus:border-white/30 transition-colors min-h-[100px] resize-none"
                    value={newPrompt}
                ...
                <div className="flex gap-3">
                  ...
                  <button 
                    ...
                    onClick={() => {
                      ...
                      handleGenerateBusiness(newPrompt);
                      setNewPrompt("");
                    }}
                    className="flex-[2] py-3 rounded-xl font-bold text-sm bg-white text-black hover:bg-gray-200 transition-colors disabled:opacity-50"
                  >
                    Launch Business
                  </button>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <h2 className="text-lg font-bold flex items-center gap-2">
            <Zap size={20} className="text-gray-400" /> Active Businesses
          </h2>
          ...
          <div className="grid grid-cols-1 gap-4">
            {businesses.length > 0 ? businesses.map((bus, i) => (
              ...
              >
                <BusinessItem 
                  name={bus.name} 
                  status={bus.status === 'active' ? 'Running' : 'Creating'} 
                  leads={String(bus.leads)} 
                  revenue={bus.revenue}
                />
              </motion.div>
            )) : (
          ...
          <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] space-y-4">
            {activities.length > 0 ? activities.slice(0, 8).map((act) => (
              ...
              <div className="text-center py-12">
                <p className="text-sm text-gray-500">No active agents. Create a business to hire AI staff.</p>
              </div>
            )}
          </div>
        </div>
...
const StatCard = ({ title, value, change }: { title: string, value: string, change: string }) => (
  <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] transition-colors cursor-default">
  ...
  <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] transition-all cursor-pointer flex items-center justify-between group">
    ...
    <div className="flex items-center gap-8 md:gap-12">
      <div className="text-right hidden sm:block">
        <p className="text-[10px] text-gray-500 uppercase font-bold tracking-tighter">Leads</p>
...
const AgentStatus = ({ name, action, time }: { name: string, action: string, time: string }) => (
  <div className="flex gap-4 items-start border-b border-white/5 pb-4 last:border-0 last:pb-0">
    ...
    <div className="text-sm">
      ...
      <p className="text-[10px] text-gray-600 mt-1 uppercase tracking-tighter font-mono">
        {time}
      </p>
    </div>
  </div>
