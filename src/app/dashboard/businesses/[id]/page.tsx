...
export default function BusinessDashboardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSaveEmails = async () => {
    setEmailSaving(true);
    await supabase.from("businesses").update({ email_outreach: emailOutreach, email_followup: emailFollowup, email_close: emailClose, followup_days: followupDays }).eq("id", id);
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
                      <div className="px-6 pt-6">
                        <div className="flex items-center justify-between mb-5">
                          <div>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">Agent fills in {"{name}"}, {"{company}"}, {"{location}"} automatically.</p>
                          </div>
                      ...
                      <div className="p-6">
                        <textarea
                          ...
                          rows={10}
                          placeholder={`Hi {name},\n\nWrite your ${activeEmailTab} email here...`}
